...
目前 Repository 以 `manifest.json` 提供最新正式更新資訊；完整 Windows Release ZIP 與 Program-only Update ZIP 只放在對應 GitHub Release 資產，不納入 Git 追蹤。
...
| GitHub Release 資產 | 不屬於 Git 追蹤檔案；完整 Windows ZIP、Program-only Update ZIP 與同內容 `manifest.json` 由 GitHub Release 提供 |
| Python Runtime | 未包含於 Repository |
| EXE | 未包含於 Repository |
| Windows 更新包 | Program-only Update ZIP 由 GitHub Release 提供，不納入 Git 追蹤 |
...
S-ERP 使用其 `version.json` 指向的 GitHub Release latest-download `manifest.json` 檢查更新。自 v0.2.8.0 New Layout baseline 起，Manifest 提供最新版、Program-only Update ZIP 下載網址、SHA-256、更新說明與發布時間；完整 Windows Release ZIP 則是同一 Release 的獨立資產。客戶端僅在使用者確認後下載並驗證更新 ZIP。更新套用、備份與回復行為由 S-ERP 原始碼 Repository 的 Launcher／Updater 實作，不在本 Repository 重複保存原始碼。
...
| 完整 Windows ZIP | 由 GitHub Release 資產提供，不納入 Git |
| Program-only Update ZIP | 由 GitHub Release 資產提供，不納入 Git |
